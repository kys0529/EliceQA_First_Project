...
def test_teamFeed_011(
    createDriver: WebDriver,
):  # 이런 음식은 싫어요! 텍스트 박스 10자 미만 텍스트 입력
    myTeamFeed = teamFeed(createDriver)
    myTeamFeed.goToPage("팀 피드")
    myTeamFeed.getElement(teamFeedLocators.TEAM_PROFILE_CHANGE).click()
...
def test_teamFeed_012(
    createDriver: WebDriver,
):  # 이런 음식은 싫어요! 텍스트 박스 100자 이상 텍스트 입력
    myTeamFeed = teamFeed(createDriver)
    myTeamFeed.goToPage("팀 피드")
    myTeamFeed.getElement(teamFeedLocators.TEAM_PROFILE_CHANGE).click()
    myTeamFeed.getElement(teamFeedLocators.TEXTAREA_HATE).clear()
...
def test_teamFeed_014(createDriver: WebDriver):  # 뒤로가기
    myTeamFeed = teamFeed(createDriver)
    myTeamFeed.goToPage("팀 피드")
    myTeamFeed.getElement(teamFeedLocators.TEAM_FEED_BACK_BTN).click()
    time.sleep(1)
...
def test_teamFeed_017(createDriver: WebDriver):  # 새로운 후기 등록하기 에서 혼밥, 그룹, 회식 버튼 클릭
    myTeamFeed = teamFeed(createDriver)
    myTeamFeed.goToPage("[+] 버튼")
    myTeamFeed.getElement(teamFeedLocators.TEAM_PLUS_ALONE_BTN).click()
    time.sleep(1)
    myTeamFeed.getElement(teamFeedLocators.TEAM_PLUS_GROUP_BTN).click()
    time.sleep(1)
    myTeamFeed.getElement(teamFeedLocators.TEAM_PLUS_TEAM_BTN).click()
    time.sleep(1)
...
def test_teamFeed_019(createDriver: WebDriver):  # [+]버튼 메뉴 등록 유효값
    try:
        myTeamFeed = teamFeed(createDriver)
        myTeamFeed.goToPage("[+] 버튼")
        myTeamFeed.getClickableElement(teamFeedLocators.TEAM_PLUS_TEAM_BTN).click()
        assert myTeamFeed.screenDiff(
            teamFeedLocators.TEAM_MENU_PLUS_IMG_INPUT,
            inspect.currentframe().f_code.co_name,
            "reviewImg",
            "image",
            myTeamFeed.getRandomImage(),
        )

        randomMenuName = (
            myTeamFeed.getRandomMenuName()
        )  # 검증이 필요하므로, 변수에 담음
        myTeamFeed.getElement(teamFeedLocators.TEAM_MENU_NAME).send_keys(randomMenuName)
        assert (
            myTeamFeed.getElement(teamFeedLocators.TEAM_MENU_NAME).get_attribute(
                "value"
            )
            == randomMenuName
        )
        ...
        assert (
            myTeamFeed.getElement(teamFeedLocators.RECOMMEND_OPTION_LABEL).text
            == randomName
        )

        randomReview = myTeamFeed.getRandomReview()  # 검증이 필요하므로, 변수에 담음
        myTeamFeed.getElement(teamFeedLocators.MY_MENU_PLUS_REVIEW_TEXTAREA).send_keys(
            randomReview
        )
        assert (
            myTeamFeed.getElement(
                teamFeedLocators.MY_MENU_PLUS_REVIEW_TEXTAREA
            ).get_attribute("value")
            == randomReview
        )
        ...
        assert (
            myTeamFeed.getElement(teamFeedLocators.TEAM_MENU_STAR_BTN).get_attribute(
                "value"
            )
            == randomValue
        )
...
def test_teamFeed_022(createDriver):
    try:
        ...
        time.sleep(1)  # 필수! (삭제 X)
        myTeamFeed.getElement(teamFeedLocators.MY_MENU_EAT_AGAIN_CANCEL_SVG).click()
        ...
        randomReview = myTeamFeed.getRandomReview()  # 검증이 필요하므로, 변수에 담음
        myTeamFeed.getElement(teamFeedLocators.TEAM_MENU_REVIEW_TEXTAREA).send_keys(
            randomReview
        )
        myTeamFeed.getElement(teamFeedLocators.TEAM_MENU_REVIEW_TEXTAREA).get_attribute(          "value"
        ) == randomReview
...
def test_teamFeed_023(createDriver):
    try:
        myTeamFeed = teamFeed(createDriver)

        myTeamFeed.goToPage("같은 메뉴 먹기")
        beforeReviewCount = myTeamFeed.getReviewPostCount()

        time.sleep(1)  # 필수! (삭제 X)
        myTeamFeed.getElement(teamFeedLocators.MY_MENU_EAT_AGAIN_CANCEL_SVG).click()

        myTeamFeed.getElement(teamFeedLocators.TEAM_MENU_PERSON_NAME_INPUT).send_keys(
            "김"
        )
        myTeamFeed.getClickableElement(
            teamFeedLocators.TEAM_MENU_PERSON_NAME_SEARCH_RESULT
        ).click()
        assert myTeamFeed.getElements(teamFeedLocators.TEAM_MENU_PERSON_REMOVE_SVG)
        ...
        assert (
            myTeamFeed.getElement(
                teamFeedLocators.TEAM_MENU_REVIEW_TEXTAREA
            ).get_attribute("value")
            == randomReview
        )
        ...
        afterReviewCount = myTeamFeed.getReviewPostCount()
...
def test_teamFeed_024(createDriver):
    try:
        myTeamFeed = teamFeed(createDriver)
        ...
        myTeamFeed.getClickableElement(
            teamFeedLocators.MY_MENU_EAT_AGAIN_CANCEL_SVG
        ).click()
        assert myTeamFeed.getElement(teamFeedLocators.TEAM_FEED_TXT)

        myTeamFeed.goToPage("같은 메뉴 먹기")
        assert (
            myTeamFeed.getElement(teamFeedLocators.TEAM_MENU_TEAM_BTN).get_attribute(
                "data-state"
            )
            == "checked"
        )
        assert (
            myTeamFeed.getElement(
                teamFeedLocators.MY_MENU_MENU_NAME_INPUT
            ).get_attribute("disabled")
            is not None
        )
        assert (
            myTeamFeed.getElement(
                teamFeedLocators.TEAM_MENU_CATEGORY_BTN
            ).get_attribute("disabled")
            is not None
        )

        myTeamFeed.getElement(teamFeedLocators.TEAM_MENU_REVIEW_TEXTAREA).clear()
        myTeamFeed.getClickableElement(teamFeedLocators.TEAM_MENU_COMPLETE_BTN).click()
