...
def test_teamFeed_007(createDriver: WebDriver):
    ...
    myTeamFeed.getElement(
        teamFeedLocators.TEAM_PROFILE_CHANGE
    ).click()  # 프로필 수정 팝업 진입
    time.sleep(1)
...
def test_teamFeed_009(
    createDriver: WebDriver,
):  # 이런 음식은 좋아요! 텍스트 박스 10자 미만 텍스트 입력
    myTeamFeed = teamFeed(createDriver)
    myTeamFeed.goToPage("팀 피드")
    myTeamFeed.getElement(teamFeedLocators.TEAM_PROFILE_CHANGE).click()
...
def test_teamFeed_010(
    createDriver: WebDriver,
):  # 이런 음식은 좋아요! 텍스트 박스 100자 이상 텍스트 입력
    myTeamFeed = teamFeed(createDriver)
    myTeamFeed.goToPage("팀 피드")
    myTeamFeed.getElement(teamFeedLocators.TEAM_PROFILE_CHANGE).click()
    myTeamFeed.getElement(teamFeedLocators.TEXTAREA_LIKE).clear()
...
def test_teamFeed_019(createDriver: WebDriver): # 유효값
    try:
        myTeamFeed = teamFeed(createDriver)
        myTeamFeed.goToPage("[+] 버튼")
        myTeamFeed.getClickableElement(teamFeedLocators.TEAM_PLUS_TEAM_BTN).click()
        assert myTeamFeed.screenDiff(teamFeedLocators.TEAM_MENU_PLUS_IMG_INPUT, inspect.currentframe().f_code.co_name, "reviewImg", "image", myTeamFeed.getRandomImage())
        ...
        randomName, randomCategory = myTeamFeed.getRandomCategory() # 검증이 필요하므로, 변수에 담음   카테고리 현재 작동이 안됨
        ...
        assert myTeamFeed.getElement(teamFeedLocators.MY_MENU_STAR_BTN).get_attribute("value") == randomValue
